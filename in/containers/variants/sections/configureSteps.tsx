    ...
    (item) => {
      ...
      if (target) {
        setSelectedItems((list) =>
          list.filter((e) =>
            e?.category === item.category &&
            e?.sub_category === item.sub_category
              ? item.name !== e?.name
              : item.hide_on !== e?.id
          )
        );
  ...
  useEffect(() => {
    if (
      ...
      preselectedItems.forEach((e: any, i) => {
        const target = pricingDetails?.items?.find(
          (each) => each?.id === e?.id
        );
          ...
          {pricingDetails?.items
            ...
            ?.filter(
              (each) =>
                ...
                each?.sub_category === sub_category
            )
            ?.map((e, i) => (
              <>
                {e.hide_on &&
                ...
                {showExchangeModal && (
                  ...
                  >
                    <div className="w-10/12 lg:w-6/12 lg:pr-20 bg-white px-10 pb-14 pt-14 relative brutal text-[12px] sm:text-[17px]">
                      Exchange programme is available for your existing
                      motorcycle . For exchange information contact{" "}
