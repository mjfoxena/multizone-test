...
/// for faq section number email link navigation

// Function to format phone numbers and email IDs
export const formatDescription = (description) => {
  // Regular expression to find phone numbers with optional dashes or spaces
  const phoneRegex = /\b(0\d{2}[-\s]?\d{3}[-\s]?\d{5})\b/g;
  // Regular expression to find email IDs
  const emailRegex = /([a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9_-]+)/g;

  // Replace phone numbers with formatted HTML with styles
  description = description.replace(phoneRegex, (match) => {
    const cleanedNumber = match.replace(/[-\s]/g, ""); // Remove dashes and spaces
    return `<a href="tel:${cleanedNumber}" style="color: red;">${match}</a>`;
  });

  // Replace email IDs with formatted HTML with styles
  description = description.replace(emailRegex, (match) => {
    return `<a href="mailto:${match}" style="color: red;">${match}</a>`;
  });
