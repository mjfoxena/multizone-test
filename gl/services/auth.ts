...
export const getAwsIdentity = async () => {
  ...
  const res = payload
    ? await cognitoFetchApi(API_CONSTANTS.COGNITO_GET_IDENTITY, "POST", payload, "id")
    : false;
...
export const getOldUserInfo = async (creds=null) => {
  ...
  const API_URL =
    API_CONSTANTS.BASE_URL +API_CONSTANTS.AWS_GET_USER_INFO;
  let payload: any = {
    email: email,
  };
  return email && payload ? await awsSignHandler(API_URL, payload, creds) : false;
};
