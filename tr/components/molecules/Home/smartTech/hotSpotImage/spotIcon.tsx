...
const SpotIcon = ({title}) => {
  ...
  return (
    <div className='cursor-pointer relative  z-50 '  >
      {(title !== '' && show ) &&  <div className={`${Style.spec} bg-[#180C2F] backdrop-filter backdrop-blur-lg bg-opacity-30 firefox:bg-opacity-90 px-4 py-2 absolute w-fit text-nowrap rounded-[2px] -top-[25%] left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-white  z-50`}>{title}</div>}
