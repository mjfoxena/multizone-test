...
const NewUser = ({name}) => {
  ...
  return (
    <div className={MapCss(Style,"background","")}>
     ...
     <div className='w-full  h-full flex flex-row'>
        ...
        <div className='w-full   h-full  flex-flex-col sm:ml-20 sm:mr-20 sm:w-2/3'>
          ...
          {/*hello pilot*/}
          <div className={MapCss(Style,"color-white brutal",'mt-8 font-normal mx-6 text-sm sm:text-xl sm:mx-0')}>
              Hello <span className='capitalize'>{name}</span>,
            <div className={MapCss(Style, "color-white", "font-normal text-xs mt-3 sm:mt-5 sm:text-base")}>
            No order placed yet.
            </div>
          </div>
          {/* configure your f77 */}
      ...
      <div className='mt-[66px] hidden  sm:flex text-end justify-end  cursor-pointer'>
            <div className={MapCss(Style,"color-white brutal","text-sm w-full")} onClick={()=>signOut()}>SIGN OUT
            </div>
     ...
     <div className='pl-6 pr-6 sm:hidden'>
            <div className={MapCss(Style,"border","mt-4")}>
            </div>
            <div className='mt-5 flex  sm:hidden   cursor-pointer'>
            <div className={MapCss(Style,"color-white brutal","text-sm")} onClick={()=>signOut()}>SIGN OUT
            </div>
    ...
    <Image src={'/images/profile/f77dark.png'} fill className={Style.f77Dark} alt='product' />
    </div>
    </div>
  )
}
